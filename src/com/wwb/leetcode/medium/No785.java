...
public class No785 {
    public boolean isBipartite(int[][] graph) {
        int n = graph.length;
        // there should be only three states
        // none, in set A (0), in set B (1)
                ...
                while (!stack.empty()) {
                    ...
                    for (int neighbor: graph[node]) {
                        if (states[neighbor] == -1) {
                            stack.push(neighbor);
                            // current node's neighbor has to be in
                            // a different set than the current node.
                            states[neighbor] = states[node] ^ 1;
                        } else if (states[neighbor] == states[node]) {
                            return false;
                        }
                    }
                }
