        ...
        while(!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int j = 0; j < currentGene.length(); j++) {
                    StringBuilder mutableGene = new StringBuilder(currentGene);
                    for (char gene : genes) {
                        if (gene == currentGene.charAt(j)) {
                            continue;
                        }

                        String mutatedGene = mutableGene.replace(j, j + 1, String.valueOf(gene)).toString();

                        if (mutatedGene.equals(endGene)) {
                            return result;
                        }

                        if (!visited.add(mutatedGene)) {
                            continue;
                        }
